<?php
    if(empty($_POST["oculto"]) || empty($_POST["txtCedula"]) || empty($_POST["txtNombre"]) || empty($_POST["txtApeUno"]) || empty($_POST["txtApeDos"]) || empty($_POST["txtTelefono"]) || empty($_POST["txtDireccion"])){
        header('Location: ../../veterinaryCatalog.php?mensaje=falta');
        exit();
    }
    ...
    if ($resultado === TRUE) {
        ...
        if($resultado2 === TRUE){
            ...
            if($resultado3 === TRUE){
                $sentencia4 = $bd->prepare("INSERT INTO veterinario (Persona_cedula) VALUES (?);");
                $resultado4 = $sentencia4->execute([$cedula]);
                if($resultado4 === TRUE){
                    header('Location: ../../veterinaryCatalog.php?mensaje=registrado');
                }else {
